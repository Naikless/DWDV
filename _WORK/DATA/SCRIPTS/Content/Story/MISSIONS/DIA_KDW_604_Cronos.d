...
{
	if((Npc_KnowsInfo(hero,KDW_604_Cronos_Greet) && ((CorAngar_SendToNC == FALSE) || Npc_KnowsInfo(hero,Info_Cronos_SLEEPER)) && (Npc_GetTrueGuild(hero) == GIL_KDW)) || ((Npc_GetTrueGuild(hero) == GIL_KDF) && Npc_KnowsInfo(hero,KDW_604_Cronos_Greet) && ((CorAngar_SendToNC == FALSE) || Npc_KnowsInfo(hero,Info_Cronos_SLEEPER))))
	{
		return TRUE;
	};
};
...
func void KDW_604_Cronos_GREET_Info()
{
	...
	AI_Output(self,other,"KDW_604_Cronos_GREET_Info_08_04");	//Allerdings werde ich erst mit dir handeln, wenn du dem Kreis des Wassers angehörst. Wir verkaufen unsere Runen nicht an Lagerfremde.
	AI_Output(self,other,"KDW_604_Cronos_GREET_Info_08_03");	//Was kann ich für dich tun?
	Log_CreateTopic(GE_TraderNC,LOG_NOTE);
	B_LogEntry(GE_TraderNC,"Cronos verkauft magische Utensilien wie RUNEN, SPRUCHROLLEN und RINGE. Er ist Tag und Nacht am Gitter über dem großen Erzhaufen zu finden. Allerdings wird er erst mit mir handeln, wenn ich dem Kreis des Wassers angehöre.");
