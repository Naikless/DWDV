...
func void Dia_Gorn_HutFree_Info()
{
	...
	AI_Output(self,other,"DIA_Gorn_HutFree_09_02");	//Pass aber auf. Wenn du dich einem anderen Lager anschließt, werden dich die anderen Söldner aus der Hütte jagen.
	AI_Output(self,other,"DIA_Gorn_HutFree_09_03");	//Die Jungs sehen es nicht gern, wenn sich Gardisten in unsere Hütten legen.
...
instance DIA_Gorn_TRADE(C_Info)
{
	...
	permanent = 1;
	description = "Hast du noch mehr von dem Zeug?";
	trade = 1;
};
...
{
	AI_Output(other,self,"DIA_Gorn_Ueberfall_15_00");	//Lares schickt mich.
	AI_Output(self,other,"DIA_Gorn_Ueberfall_09_01");	//Du bist also das neue Mitglied der Bande?
	AI_Output(other,self,"DIA_Gorn_Ueberfall_15_02");	//Ja, aber warum beteiligst du dich an den Überfällen?
	AI_Output(self,other,"DIA_Gorn_Ueberfall_09_03");	//Wenn ich das tun würde, könnte ich dir das nicht sagen. Lee sieht sowas nämlich gar nicht gerne.
	AI_Output(other,self,"DIA_Gorn_Ueberfall_15_04");	//Verstehe.
	AI_Output(self,other,"DIA_Gorn_Ueberfall_09_05");	//Können wir dann los?
	AI_Output(other,self,"DIA_Gorn_Ueberfall_15_06");	//Los geht's!
	...
	B_LogEntry(CH2_KONVOINC,"Gorn, einer der Söldner der Wassermagier, ist einer derjenigen, die den Überfall auf den Konvoi der alten Mine anführen. Nun soll ich mit ihm zusammen den Konvoi überfallen.");
};
...
{
	AI_Output(self,other,"DIA_Gorn_Done_15_00");	//Scheinst ja ein guter Kämpfer zu sein.
	AI_Output(self,other,"DIA_Gorn_Done_15_01");	//Ich werd' auch mal wieder zurück ins neue Lager gehen.
	AI_Output(self,other,"DIA_Gorn_Done_15_02");	//Ich denke, wir werden uns nicht das letzte mal gesehen haben, was?
	AI_Output(self,other,"DIA_Gorn_Done_15_03");	//Ich hab' noch mehr Zeug von Überfällen. Wenn du handeln willst, frag mich einfach danach.
	AI_Output(self,other,"DIA_Gorn_Done_15_04");	//Wie auch immer, bis zum nächsten mal.
	...
	B_LogEntry(CH2_KONVOINC,"Gorn war beeindruckt von meinen Fähigkeiten und war der Meinung das ich gute Arbeit geleistet hätte. Nach dem Überfall kehrte er ins Lager zurück.");
};
