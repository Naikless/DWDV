...
func int ZS_StandAround_Loop()
{
	...
	if(self.aivar[AIV_ITEMSTATUS] == TA_IT_NONE)
	{
		...
		else if(Npc_GetTrueGuild(self) == GIL_None)
		{
			if(choice < 20)
			{
				B_ChooseMeat(self);
		...
		else if(Npc_GetTrueGuild(self) == GIL_SHADOWBEAST)
		{
			B_SetPerception(self);
			if(choice < 10)
			{
				AI_PlayAni(self,"R_SCRATCH2");
			}
			else if(choice < 20)
			{
