...
{
	AI_Output(other,self,"Info_Kharim_Charge_Insult_Face_15_00");	//Ich wollte zuerst was über deine dürren Ärmchen sagen, aber dann fiel mir deine hässliche Fresse auf.
...
instance INFO_KHARIM_ARENAFIGHT(C_Info)
{
	npc = Sld_729_Kharim;
	nr = 0;
	condition = info_kharim_arenafight_condition;
	information = info_kharim_arenafight_info;
	important = 1;
	permanent = 1;
};


func int info_kharim_arenafight_condition()
{
	if((KHARIM_FIGHT == TRUE) && (Npc_GetDistToWP(hero,"OCR_ARENABATTLE_TRAIN") < 500) && ((Sld_729_Kharim.aivar[AIV_HASDEFEATEDSC] == FALSE) || (Sld_729_Kharim.aivar[AIV_WASDEFEATEDBYSC] == FALSE)))
	{
		return 1;
	};
};

func void info_kharim_arenafight_info()
{
	AI_Output(self,other,"Info_Kharim_InArena_09_00");	//Schmerzen sind eine Frage des Willens!
	AI_StopProcessInfos(self);
	Npc_ExchangeRoutine(self,"START");
