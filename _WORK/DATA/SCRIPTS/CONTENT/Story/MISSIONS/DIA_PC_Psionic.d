...
{
	if(self.aivar[AIV_PARTYMEMBER] || (Kapitel > 2))
	{
...
func void PC_Psionic_GUIDEFIRST_Info()
{
	...
	Info_AddChoice(PC_Psionic_GUIDEFIRST,"zur freien Hütte",pc_psionic_guidefirst_hut);
};
...
func void pc_psionic_guidefirst_hut()
{
	AI_Output(other,self,"PC_Psionic_GUIDEFIRST_HUT_Info_15_01");	//... zur freien Hütte!
	AI_Output(self,other,"PC_Psionic_GUIDEFIRST_HERB_Info_05_02");	//folge mir!
	LesterGuide = 5;
	AI_StopProcessInfos(self);
	Npc_ExchangeRoutine(self,"GUIDETOHEROHUT");
};
...
instance PC_PSIONIC_HUT(C_Info)
{
	npc = PC_Psionic;
	condition = pc_psionic_hut_condition;
	information = pc_psionic_hut_info;
	important = 1;
	permanent = 0;
};


func int pc_psionic_hut_condition()
{
	if((Npc_GetDistToWP(self,"PSI_PATH_4_11") < 800) && (LesterGuide == 5))
	{
		return TRUE;
	};
};

func void pc_psionic_hut_info()
{
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_10");	//Wenn du diese Leiter hinaufsteigst, kommst du zu der Hütte.
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_12");	//Allerdings lassen wir normalerweise nur Lagerangehörige oder Neue in unseren Hütten schlafen.
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_13");	//Also pass auf, dass du dich nicht als Angehöriger eines anderen Lagers in unsere Hütten legst.
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_14");	//Die Templer können ziemlich unfreundlich werden, wenn du dich nicht an unsere Regeln hälst.
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_11");	//Sieh dich ein wenig um, ich werde hier auf dich warten. Aber lass dir nicht zu viel Zeit, sonst gehe ich wieder zurück.
	LesterGuide = 0;
	Npc_ExchangeRoutine(self,"START");
	AI_StopProcessInfos(self);
	TA_BeginOverlay(self);
	TA_Stay(0,0,1,0,"PSI_PATH_4_11");
	TA_EndOverlay(self);
};
...
func void PC_Psionic_BROTHERHOOD_TODO_Info()
{
	...
	{
		AI_Output(self,other,"PC_Psionic_BROTHERHOOD_TODO_05_05");	//Sprich mit Baal Namib. Als einer unserer Novizen brauchst du die Erlaubnis eines Gurus den Tempelberg zu betreten. Er wird die Wachen anweisen dich passieren zu lassen.
	};
...
func void Info_Lester_DIEGOMILTEN_Info()
{
	...
	Npc_ExchangeRoutine(PC_Psionic,"MEETING");
...
instance DIA_LESTER_MYOWNHUT(C_Info)
{
	npc = PC_Psionic;
	nr = 10;
	condition = dia_lester_ownhut_condition;
	information = dia_lester_ownhut_info;
	permanent = 1;
	description = "Kann ich hier auch irgendwo schlafen?";
};


func int dia_lester_ownhut_condition()
{
	if((Npc_KnowsInfo(hero,DIA_Lester_Hallo) || Npc_KnowsInfo(hero,DIA_Lester_Sakrileg)) && (Kapitel < 3))
	{
		return 1;
	};
};

func void dia_lester_ownhut_info()
{
	AI_Output(other,self,"DIA_Lester_Hut_12_10");	//Kann ich hier auch irgendwo schlafen?
	AI_Output(self,other,"DIA_Lester_Hut_12_20");	//Ja, eine der Hütten in der Nähe vom Lagerzentrum ist vor kurzem frei geworden.
	AI_Output(self,other,"DIA_Lester_Hut_12_30");	//Wenn du willst, kann ich dich hinführen und dir zeigen wo sie steht.
};
