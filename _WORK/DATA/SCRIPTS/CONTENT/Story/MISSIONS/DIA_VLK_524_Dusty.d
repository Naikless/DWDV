...
instance DIA_DUSTY_DABEI(C_Info)
{
	npc = VLK_524_Dusty;
	nr = 1;
	condition = dia_dusty_dabei_condition;
	information = dia_dusty_dabei_info;
	permanent = 0;
	important = 1;
};


func int dia_dusty_dabei_condition()
{
...
func void dia_dusty_dabei_info()
{
	var C_Npc Dusty;
	Dusty = Hlp_GetNpc(VLK_524_Dusty);
	AI_Output(self,other,"DIA_BaalTondral_DustySuccess_03_05");	//Heißt das, ich bin dabei - einfach so?
	AI_Output(other,self,"DIA_BaalTondral_DustySuccess_15_06");	//Sieht so aus. Tu einfach, was der Meister dir sagt.
	AI_StopProcessInfos(self);
	Dusty.aivar[AIV_PARTYMEMBER] = FALSE;
	Dusty.guild = GIL_NOV;
	Npc_SetTrueGuild(Dusty,GIL_NOV);
	B_ExchangeRoutine(VLK_524_Dusty,"PREPARERITUAL");
	B_ExchangeRoutine(VLK_582_Melvin,"PREPARERITUAL");
};
