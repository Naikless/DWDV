...
func void Info_Bloodwyn_PayForJesse_Info()
{
	...
	if(Npc_HasItems(other,ItMiNugget) >= 10)
	{
		...
		B_GiveInvItems(other,self,ItMiNugget,10);
		B_LogEntry(CH1_JESSE,"Ich habe Bloodwyn die 10 Erz von Jesse gezahlt. Allerdings meinte er, ich solle zusehen das ich lieber mal MEINE Summe bezahle.");
		AI_StopProcessInfos(self);
	}
	else
	{
...
{
	AI_Output(self,other,"GRD_233_Bloodwyn_GETOUT_Info_08_01");	//So einer wie du hätte es weit bringen können. Stattdessen bist du zu den Reisfressern ins Neue Lager gegangen. Verschwinde!
	AI_StopProcessInfos(self);
};
