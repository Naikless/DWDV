...
func void B_MM_AssessBody()
{
	...
	if(self.aivar[AIV_PLUNDERED] == PRIO_PREY)
	{
		...
		{
			if(C_PreyToPredator(other,self) || (C_NpcIsHuman(other) && ((self.guild == GIL_SNAPPER) || (self.guild == GIL_WOLF) || (self.guild == GIL_ORCDOG) || (self.guild == GIL_SHADOWBEAST))))
			{
				Npc_ClearAIQueue(self);
...
func int ZS_MM_Attack_Loop()
{
	...
	if(Hlp_IsValidNpc(other) && !C_NpcIsDown(other))
	{
		...
		else if(C_BodyStateContains(other,BS_SWIM) || C_BodyStateContains(other,BS_DIVE))
		{
			...
			{
				PrintDebugNpc(PD_MST_CHECK,"...ich kann nicht schwimmen!");
				return 1;
			};
		}
		else
		...
		{
			if(self.aivar[AIV_LASTHITBYRANGEDWEAPON] == TRUE)
			{
				PrintDebugNpc(PD_MST_CHECK,"...ich wurde von einem Pfeil getroffen!");
			};
			PrintDebugNpc(PD_MST_CHECK,"...Attack!");
			AI_Attack(self);
		};
	}
	else
...
func void B_MM_AssessWarn()
{
	...
	if((Wld_GetGuildAttitude(self.guild,other.guild) == ATT_FRIENDLY) && (self.aivar[AIV_FINDABLE] == PACKHUNTER) && (Npc_GetDistToNpc(self,other) <= self.aivar[AIV_PCISSTRONGER]))
	{
