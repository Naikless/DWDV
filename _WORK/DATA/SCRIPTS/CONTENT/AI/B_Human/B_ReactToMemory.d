...
func void B_ReactToMemory()
{
	...
	if(murdernews > 0)
	{
		...
		pipe = ConcatStrings("...murdernews_victim:   ",murdernews_victim.name);
		PrintDebugNpc(PD_ZS_Check,pipe);
		pipe = ConcatStrings("...murdernews_offender:   ",murdernews_offender.name);
		PrintDebugNpc(PD_ZS_Check,pipe);
		if(B_CompareNpcInstance(murdernews_offender,self))
		{
			PrintDebugNpc(PD_ZS_Check,"B_ReactToMemory // Murder_News // NPC Offender");
		...
		if(B_CompareNpcInstance(murdernews_offender,other))
		{
			...
			if(Npc_GetAttitude(self,murdernews_victim) == ATT_HOSTILE)
			{
				PrintDebugNpc(PD_ZS_Check,"B_ReactToMemory // Murder_News // NPC hostile to victim");
				Npc_DeleteNews(self,murdernews);
				return;
			};
		...
		PrintDebugNpc(PD_ZS_Check,"B_ReactToMemory // Murder_News // else");
		Npc_DeleteNews(self,murdernews);
	};
	...
	if(defeat_news > 0)
	{
		...
		PrintGlobals(PD_ZS_Check);
		pipe = ConcatStrings("...defeat_news_offender:   ",defeat_news_offender.name);
		PrintDebugNpc(PD_ZS_Check,pipe);
		pipe = ConcatStrings("...defeat_news_victim:   ",defeat_news_victim.name);
		PrintDebugNpc(PD_ZS_Check,pipe);
		if(B_CompareNpcInstance(defeat_news_offender,self))
		{
			PrintDebugNpc(PD_ZS_Check,"B_ReactToMemory // Defeat_News // NPC Offender");
		...
		if(B_CompareNpcInstance(other,defeat_news_offender))
		{
			...
			{
				PrintDebugNpc(PD_ZS_Check,"B_ReactToMemory // Defeat_News // SC offender// victim is an other");
				if((Npc_GetAttitude(self,defeat_news_victim) == ATT_ANGRY) || (Npc_GetAttitude(self,defeat_news_victim) == ATT_HOSTILE))
				{
					PrintDebugNpc(PD_ZS_Check,"B_ReactToMemory // Defeat_News // NPC doesn't care about victim");
					Npc_DeleteNews(self,defeat_news);
					return;
				};
				if(Npc_GetPermAttitude(self,defeat_news_offender) == ATT_ANGRY)
		...
		PrintDebugNpc(PD_ZS_Check,"B_ReactToMemory // Defeat_News // else");
		Npc_DeleteNews(self,defeat_news);
	};
